...
import { Question, decodeQuestion, encodeQuestion } from './question';

const encodeEntries: { input: Question; base64: string }[] = [
  {
    input: {
...
const decodeEntries: { input: Buffer; expected: Question }[] = [
  {
    input: Buffer.from([
  ...
  describe('encodeQuestion()', () => {
    it('should correctly encode question entry to octect array', () => {
      encodeEntries.forEach(({ input, base64 }) => {
        expect(encodeQuestion(input).toString('base64')).toBe(base64);
      });
    });
  });
  ...
  describe('decodeQuestion()', () => {
    it('should correctly decode question buffer to question object', () => {
      decodeEntries.forEach(({ input, expected }) => {
        const question = decodeQuestion(input);
